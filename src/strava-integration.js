class StravaIntegration {
    ...
    renderActivities() {
        ...
        container.innerHTML = this.activities.map((activity, index) => `
            <div class="activity-section" data-activity-id="${activity.id}">
                ...
                <div class="activity-photos">
                    <div class="photo-count">${activity.total_photo_count} photos</div>
        ...
        // Load photos for only the first 4 activities (most recent)
    ...
    async importFullActivity(activityId) {
        try {
            ...
            // Fetch detailed activity data with photos
    ...
    async loadRecentActivityPhotos() {
        // Load photos for only the first 4 activities (most recent) to avoid rate limiting
        const recentActivities = this.activities.slice(0, 4);
        const promises = recentActivities.map(activity => 
            this.loadActivityPhotos(activity.id).catch(error => {
    ...
    async loadActivityPhotos(activityId) {
        try {
            ...
            if (activity.photos && activity.photos.length > 0) {
                photosContainer.innerHTML = activity.photos.map(photo => {
